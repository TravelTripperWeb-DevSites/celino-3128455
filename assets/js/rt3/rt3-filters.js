...
.filter('fetchRoomRate', function () {
  return function (roomsList, roomCode, showCheckAvailability) {
    var selectedRoom;
    if(roomCode && roomsList) {
      selectedRoom = roomsList.filter(function(v, i) {
        return v.code == roomCode;
      });
      if(selectedRoom[0] && selectedRoom[0].todayRate != 'CHECK AVAILABILITY') {
        return 'From $' + selectedRoom[0].todayRate;
      } else if(showCheckAvailability) {
